# Zotify Downloader wrapper - MP3 FORMAT
...
{
		write-host "Usage: ZD3 `<spotify URL`>"
		exit
}
...
$output = switch ($DLDtype) {
   "playlist" { "{playlist}/{playlist_num} - {artist} - {song_name}.mp3" ; break}
   "artist"   { "{artist}/{album}/{track_number} - {song_name}.mp3" ; break}
   "album"    { "{artist}/{album}/{track_number} - {song_name}.mp3" ; break}
   "track"    { "{artist}/{album} - {track_number} - {song_name}..mp3" ; break}
   "episode"  { "{artist}/{song_name}.mp3"; break}
...
do {
	...
	">>>> ITERATION $iteration <<<<"
	$("=" * 80)
	zotify --root-path $rootpath --output $output --retry-attempts 5 --print-download-progress=false --download-lyrics=false --download-format mp3 --download-quality high $spoturi 2>&1 | Tee-Object -file $logfilename
